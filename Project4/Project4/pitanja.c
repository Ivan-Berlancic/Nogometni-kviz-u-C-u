...
int generiranje_pitanja(int* polje) {
	...
	case 1:
		...
		if (_getch() == 'B')
		{
			...
			if (countr == 500) {
				printf("\nOsvojili ste %d bodova!", countr); rezultat(countr); j = 0; countr = 0; _getch();
				return 1;
			}
		...
		else
		{
			...
			printf("\nOsvojili ste %d bodova!", countr); rezultat(countr); countr = 0; _getch();
			return 1;
		}
		break;
	case 2:
...
case 12:
	...
	printf("\n");
	printf("\n\nC - Westfalenstadion                     D - Park Prinèeva     ");
	printf("\n\n----------------------------------------------------------------");
	if (_getch() == 'A')
	{
		printf("\n\nTocan odgovor!\nPritisnite bilo koju tipku da biste nastavili."); j++; countr += 100; _getch();
